...
Page({
  ...
  async getGoodsDetail(goods_id) {
    ...
    // 2 判断当前商品是否被收藏
    let isCollect= collect.some(v => v.goods_id === this.goodsDetailList.goods_id)
    // let isCollect
    // if(collect.length>0){
    //   isCollect= collect.some(v => v.goods_id === this.goodsDetailList.goods_id)
    // }else{
    //   isCollect=false
    // }
  ...
  handleCollect() {
    ...
    let collect = wx.getStorageSync("collect") || [];
    // 2 判断当前商品是否被收藏
    let index = collect.findIndex(v => v.goods_id === this.goodsDetailList.goods_id)
    // 3 当index！=-1表示 已经收藏过 
    if (index !== -1) {
      ...
      collect.push(this.goodsDetailList)
      wx.showToast({
        title: '收藏成功',
        icon: 'success',
        mask: false,
