...
 ERR_PNPM_FETCH_404  GET https://registry.npmjs.org/aixos: Not Found - 404
...
要有git仓库，如果没有git仓库，可以先执行git init，初始化仓库
...
添加配置文件，手动新建`commitlint.config.cjs`(注意是cjs)，然后添加下面的代码：
...
配置husky （在husky这个目录下面会有一个commit-msg文件）
